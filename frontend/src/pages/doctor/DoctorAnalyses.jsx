...
function DoctorAnalyses() {
    ...
    const getDiagnosisDisplayTotal = (diagnosis) => {
        const items = Array.isArray(diagnosis?.diagnosisPrices) ? diagnosis.diagnosisPrices : []
        const subtotal = items.reduce((sum, item) => sum + Number(item?.price || 0), 0)
        return subtotal > 0 ? subtotal : Number(diagnosis?.totalAmount || 0)
    }
                        ...
                        <tbody>
                            {paged.map((d, index) => (
                                <tr key={d._id} className={isToday(d.createdAt) ? 'pm-today-row' : ''}>
                                    ...
                                    <td><strong>{getDiagnosisDisplayTotal(d).toLocaleString()} so'm</strong></td>
