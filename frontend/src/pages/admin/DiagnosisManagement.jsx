...
function DiagnosisManagement() {
    ...
    const handleDrop = useCallback((e, dropIndex) => {
        ...
        // Faqat shu categoryning analizlarini olish
        setLocalDiagnoses(prev => {
            const newList = [...prev]
            // filteredDiagnoses indekslarini original ro'yxatdagi indekslarga aylantirish
            const fromItem = filteredDiagnoses[dragIndex]
            const toItem = filteredDiagnoses[dropIndex]
            const fromRealIdx = newList.findIndex(d => d._id === fromItem._id)
            const toRealIdx = newList.findIndex(d => d._id === toItem._id)

            // Joyni almashtirish
            newList.splice(fromRealIdx, 1)
            newList.splice(toRealIdx, 0, fromItem)
            return newList
        })
        ...
        // Serverga saqlash (debounce bilan)
        if (reorderTimeoutRef.current) clearTimeout(reorderTimeoutRef.current)
        reorderTimeoutRef.current = setTimeout(async () => {
            setReorderSaving(true)
            try {
                const token = localStorage.getItem('token')
                // Yangilangan localDiagnoses'dan faqat shu categoryга tegishli IDlarni olish
                setLocalDiagnoses(current => {
                    const catIds = current
                        .filter(d => {
                            const catId = d.category?._id || d.category
                            return selectedCategory
                                ? catId?.toString() === selectedCategory._id?.toString()
                                : true
                        })
                        .map(d => d._id)

                    fetch('/api/diagnoses/reorder/batch', {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${token}`
                        },
                        body: JSON.stringify({ orderedIds: catIds })
    ...
    }, [dragIndex, filteredDiagnoses, selectedCategory])
