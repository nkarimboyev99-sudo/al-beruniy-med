...
function PatientManagement({ readOnly = false }) {
    ...
    const fetchCategoriesList = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const firstVisibleCategory = data.find(cat => !isInternalResultCategory(cat))
    ...
    const getCategoryById = (categoryId) =>
        categoriesList.find(c => toId(c._id) === toId(categoryId))

    const getCategoryByName = (name) =>
        categoriesList.find(c => normalizeName(c.name) === normalizeName(name))

    const getCategoryByCode = (code) =>
        categoriesList.find(c => normalizeName(c.code) === normalizeName(code))

    const getResultItemKey = (item) => {
        const id = toId(item?.id)
        if (id) return `diagnosis:${id}`

        const categoryId = toId(item?.categoryId)
        const code = normalizeName(item?.code)
        if (categoryId && code) return `category:${categoryId}:code:${code}`

        const name = normalizeName(item?.displayName || item?.name)
        if (categoryId && name) return `category:${categoryId}:name:${name}`

        return name || code
    }

    const addResultSourceItem = (items, seenKeys, item) => {
        if (!item?.displayName) return
        const cat = getCategoryById(item.categoryId)
        const catCode = normalizeName(cat?.code)
        const catName = normalizeName(cat?.name)
        const isOam = catCode === 'oam' || catName === normalizeName('общий анализ мочи')
        const code = normalizeName(item.code)
        const name = normalizeName(item.displayName || item.name)
        if (isOam && (code === 'oam-leuco' || name === 'лейкоциты')) return
    ...
    const hasCategoryInSource = (source, sourceItems, categoryCode, categoryName) => {
        const normCode = normalizeName(categoryCode)
        const normName = normalizeName(categoryName)
        const sourceCategory = source?.diagnosis?.category
        if (normalizeName(sourceCategory?.code) === normCode || normalizeName(sourceCategory?.name) === normName) return true
        if ((source?.diagnosisName || '').split(',').map(name => normalizeName(name.trim())).includes(normName)) return true
        return sourceItems.some(item => {
            const category = getCategoryById(item.categoryId)
            return normalizeName(category?.code) === normCode || normalizeName(category?.name) === normName
        })
    }

    const ensureMandatoryLeukocyteItems = (source, sourceItems, seenKeys) => {
        const addByCode = (categoryCode, categoryName, diagnosisCode, fallbackName, fallbackOrder) => {
            if (!hasCategoryInSource(source, sourceItems, categoryCode, categoryName)) return
            const category = getCategoryByCode(categoryCode) || getCategoryByName(categoryName)
            const categoryId = toId(category?._id)
            const match = findDiagnosisByCode(diagnosisCode, categoryId)
            const code = match?.code || diagnosisCode
            const displayName = match?.name || fallbackName
            const item = {
                id: toId(match?._id),
                name: normalizeName(displayName),
                displayName,
                categoryId: getDiagnosisCategoryId(match) || categoryId,
                code,
                order: getCanonicalOrder(code, match?.order ?? fallbackOrder)
            }
            addResultSourceItem(sourceItems, seenKeys, item)
        }

        addByCode('OAK', 'Общий анализ крови', 'WBC', 'Лейкоциты', 40.0)
    }

    const toResultSourceItem = (diagnosis, fallbackCategoryId = null) => {
        const displayName = diagnosis?.name || ''
        const code = diagnosis?.code || getCodeByKnownName(displayName)
        return {
            id: toId(diagnosis?._id),
            name: normalizeName(displayName),
            displayName,
            categoryId: getDiagnosisCategoryId(diagnosis) || toId(fallbackCategoryId),
            code,
            order: getCanonicalOrder(code, diagnosis?.order)
        }
    }

    const getDiagnosesForResultCategory = (categoryId) =>
        diagnosesList
            .filter(d => getDiagnosisCategoryId(d) === toId(categoryId))
            .sort((a, b) => {
    ...
    const getSelectedPackageCategoryIds = (source) => {
        const categoryIds = new Set()
        const addPackageCategoryId = (value) => {
            const id = toId(value)
            if (!id) return
            const cat = getCategoryById(id)
            if (cat && cat.hideAnalyses) {
                categoryIds.add(id)
            }
        }

        ;(source?.diagnosisPrices || []).forEach(dp => {
            const match = findDiagnosisByRef(dp.diagnosisId, dp.name, dp.categoryId)
            const catId = toId(dp.categoryId) || getDiagnosisCategoryId(match)

            if (dp?.isCategoryPrice && catId) {
                categoryIds.add(catId)
            } else if (catId) {
                addPackageCategoryId(catId)
            }

            const cat = getCategoryById(catId) || getCategoryByName(dp.categoryName) || getCategoryByName(dp.name)
            if (cat && (dp?.isCategoryPrice || cat.hideAnalyses)) {
                categoryIds.add(cat._id)
            }
        })

        const tags = (source?.diagnosisName || '')
            .split(',')
            .map(name => name.trim())
            .filter(Boolean)
        ...
        tags.forEach(tag => {
            ...
            const category = getCategoryByName(tag) || getCategoryByName(cleanTag)
            if (category && category.hideAnalyses) {
                categoryIds.add(category._id)
            }
        })

        const oamCategory = getCategoryByCode('OAM') || getCategoryByName('Общий анализ мочи')
        const momCategory = getCategoryByCode('MOM') || getCategoryByName('Микроскопия осадка мочи')
        if (oamCategory && momCategory && categoryIds.has(toId(oamCategory._id))) {
            categoryIds.add(momCategory._id)
        }

        return Array.from(categoryIds)
    }

    const getResultSourceItems = (source) => {
        const seenKeys = new Set()
        const sourceItems = []
        const selectedPackageCategoryIds = new Set(getSelectedPackageCategoryIds(source))
        const selectedDiagnosisNames = new Set(
        ...
        const shouldKeepPurchasedItem = (item) => {
            const cat = getCategoryById(item.categoryId)
            const isMom = isInternalResultCategory(cat) || (momCatId && toId(item.categoryId) === momCatId)

            // Strictly remove MOM items if MOM was not selected
            if (isMom && selectedPackageCategoryIds.size > 0 && momCatId && !selectedPackageCategoryIds.has(momCatId)) {
                return false
            }

            if (selectedPackageCategoryIds.size === 0) return true
            const categoryId = toId(item.categoryId)
            if (categoryId && selectedPackageCategoryIds.has(categoryId)) return true
            return selectedDiagnosisNames.has(item.name)
        }

        ;(source?.diagnosisPrices || [])
            .filter(dp => !dp.isCategoryPrice)
            .map(dp => {
                const match = findDiagnosisByRef(dp.diagnosisId, dp.name, dp.categoryId)
                const displayName = match?.name || dp.name
                const code = match?.code || dp.code || getCodeByKnownName(displayName)
                return {
                    ...
                    code,
                    order: getCanonicalOrder(code, match?.order ?? dp.order)
                }
            })
            .filter(Boolean)
            .filter(shouldKeepPurchasedItem)
            .forEach(item => addResultSourceItem(sourceItems, seenKeys, item))

        const fallbackTags = (source?.diagnosisName || '')
            .split(',')
            .map(name => name.trim())
            .filter(Boolean)

        if (sourceItems.length === 0) {
            fallbackTags.forEach(displayName => {
                if (getCategoryByName(displayName)) return

                const match = findDiagnosisByRef(
                    null,
                    displayName,
                    source?.diagnosis?.category?._id || source?.diagnosis?.category
                )
                const finalDisplayName = match?.name || displayName
                const code = match?.code || getCodeByKnownName(finalDisplayName)
                const item = {
                    id: toId(match?._id),
                    name: normalizeName(finalDisplayName),
                    displayName: finalDisplayName,
                    categoryId: getDiagnosisCategoryId(match) || toId(source?.diagnosis?.category?._id || source?.diagnosis?.category),
                    code,
                    order: getCanonicalOrder(code, match?.order)
                }
                addResultSourceItem(sourceItems, seenKeys, item)
            })
        }

        ensureMandatoryLeukocyteItems(source, sourceItems, seenKeys)

        // Global name dedup: har xil categoryId bo'lsa ham, bir xil nomli elementlarni bitta qoldirish
        const seenSourceNames = new Set()
        const dedupedItems = sourceItems.filter(item => {
            const n = normalizeName(item.displayName || item.name)
            if (n && seenSourceNames.has(n)) return false
            if (n) seenSourceNames.add(n)
            return true
        })
        ...
        return dedupedItems.sort((a, b) => {
            ...
            if (orderA !== orderB) return orderA - orderB
            return normalizeName(a.displayName).localeCompare(normalizeName(b.displayName))
        })
    }


    const normalizeSavedResultRows = (rows, cols, sourceDiagnosis = null) => {
        ...
        visibleRows.forEach((row, idx) => {
            ...
            const key = (rowCategory && rowName ? `${rowCategory}:${rowName}` : null) || rowId || rowName || row.id || `row-${idx}`
            if (!byKey.has(key)) {
    ...
    const buildAutoRows = (diagnosisName, patient = null, savedRows = [], columns = null, sourceDiagnosis = null, filterCategoryId = null) => {
        ...
        let sourceItems = getResultSourceItems(sourceDiagnosis || { diagnosisName })
        if (filterCategoryId) {
            sourceItems = sourceItems.filter(item => toId(item.categoryId) === toId(filterCategoryId))
        }

        if (tags.length === 0 && sourceItems.length === 0) {
            const empty = {}
            cols.forEach(c => { empty[c.id] = '' })
            return [{ id: Date.now(), values: empty }]
        }

        const usedSourceItemIndexes = new Set()

        const uniqueEntries = []
        const seenEntryKeys = new Set()
        const canonicalEntries = sourceItems.length > 0
            ? sourceItems.map((item, sourceIndex) => ({
                tag: item.displayName || item.name,
                sourceItem: item,
                sourceIndex
            }))
            : tags.map(tag => ({ tag }))
        ...
        canonicalEntries.forEach(entry => {
            ...
            if (!tagNorm) return
            const key = entry.sourceItem
                ? getResultItemKey(entry.sourceItem) || `${toId(entry.sourceItem.categoryId)}:${tagNorm}:${entry.sourceIndex}`
                : `tag:${tagNorm}`
            if (!key || seenEntryKeys.has(key)) return
            seenEntryKeys.add(key)
            uniqueEntries.push({ ...entry, tag })
        })

        const autoRows = uniqueEntries.map((entry, i) => {
            const tag = entry.tag
            // Find the most specific sourceItem: prefer one that matches both name AND saved row's categoryId
            const tagNorm = normalizeName(tag)
            let sourceItem = entry.sourceItem
            if (sourceItem) {
                usedSourceItemIndexes.add(entry.sourceIndex)
            } else {
                const allMatching = sourceItems
                    .map((item, sourceIndex) => ({ item, sourceIndex }))
                    .filter(({ item }) => item.name === tagNorm)
                // Also check if there's a saved row with a _categoryId hint for this tag
                const savedRowHint = savedRows.find(r => {
                    const values = r.values || {}
                    return normalizeName(values[idName]) === tagNorm && toId(values._categoryId)
                })
                const savedCategoryIdHint = savedRowHint ? toId(savedRowHint.values?._categoryId) : null
                let sourceMatch = allMatching.find(({ item, sourceIndex }) =>
                    savedCategoryIdHint &&
                    !usedSourceItemIndexes.has(sourceIndex) &&
                    toId(item.categoryId) === savedCategoryIdHint
                )
                if (!sourceMatch) {
                    sourceMatch = allMatching.find(({ sourceIndex }) => !usedSourceItemIndexes.has(sourceIndex))
                }
                if (sourceMatch) usedSourceItemIndexes.add(sourceMatch.sourceIndex)
                sourceItem = sourceMatch?.item
            }
            const match = findDiagnosisByRef(sourceItem?.id, tag, sourceItem?.categoryId)
            let norma = '', result = '', unit = ''
            if (match && match.normalRanges && match.normalRanges.length > 0) {
                ...
                norma = found.range || ''
                result = looksCorruptedText(found.result) ? '' : (found.result || '')
                unit = found.unit || ''
            }
            const matchId = toId(match?._id || sourceItem?.id)
            // filterCategoryId berilganda, kategoriyani shu filter dan olamiz — noto'g'ri category nomi chiqmasligi uchun
            const matchCategoryId = filterCategoryId
                ? filterCategoryId
                : getDiagnosisCategoryId(match) || toId(sourceItem?.categoryId)
            const sourceCategoryName = getCategoryById(sourceItem?.categoryId)?.name || sourceDiagnosis?.diagnosis?.category?.name || ''
            const matchCategoryName = filterCategoryId
                ? (categoriesList.find(c => c._id === filterCategoryId)?.name || getDiagnosisCategoryName(match, sourceDiagnosis?.diagnosis?.category?.name || ''))
                : getDiagnosisCategoryName(match, sourceCategoryName)
            // Avval saqlangan Результат qiymatini saqlab qolish (id/category bo'yicha, fallback nom bo'yicha)
            const savedRow = savedRows.find(r => {
                const values = r.values || {}
                const rowId = toId(values._diagnosisId)
                const rowCategoryId = toId(values._categoryId)
                const rowName = normalizeName(values[idName])
                const tagName = normalizeName(tag)
                return (matchId && rowId === matchId) ||
                    (matchCategoryId && rowCategoryId === matchCategoryId && rowName === tagName) ||
                    (!rowId && !rowCategoryId && rowName === tagName)
            })
            const savedResult = savedRow?.values?.[idResult]
            const rezultat = savedResult !== undefined && savedResult !== null
                ? (looksCorruptedText(savedResult) ? '' : savedResult)
                : result
            return {
                id: Date.now() + i + Math.random(),
                values: {
                    _diagnosisId: matchId,
                    _categoryId: matchCategoryId,
                    _categoryName: matchCategoryName,
                    [idName]: tag,
                    [idResult]: rezultat,
                    [idNorma]: norma,
                    [idUnit]: unit
        ...
        const selectedPackageCategoryIdsSet = new Set(getSelectedPackageCategoryIds(sourceDiagnosis || { diagnosisName }))
        const momCategory = getCategoryByCode('MOM') || getCategoryByName('Микроскопия осадка мочи')
        const momCatId = toId(momCategory?._id)

        const seenNames = new Set()
        return autoRows.filter(row => {
            const vals = row.values || {}
            const rowName = normalizeName(vals[idName])
            const rowCatId = toId(vals._categoryId)
            const cat = getCategoryById(rowCatId)
            const isMom = isInternalResultCategory(cat) || (momCatId && rowCatId === momCatId)

            // Strictly filter out MOM rows if MOM category was not selected in package
            if (isMom && selectedPackageCategoryIdsSet.size > 0 && momCatId && !selectedPackageCategoryIdsSet.has(momCatId)) {
                return false
            }

            // Global name dedup: skip any row with same name already seen
            if (rowName && seenNames.has(rowName)) {
                return false
            }
            if (rowName) seenNames.add(rowName)
    ...
    const getResultTableNames = (diagnosis, sourceItems) => {
        const names = []
        const seen = new Set()
        const addName = (name) => {
        ...
        getSelectedPackageCategoryIds(diagnosis).forEach(categoryId => {
            addName(getCategoryById(categoryId)?.name)
        })

        sourceItems.forEach(item => {
            addName(getCategoryById(item.categoryId)?.name)
        })
        ...
        if (names.length === 0) {
            addName(diagnosis?.diagnosis?.category?.name || diagnosis?.diagnosisName || 'Natija')
        }

        return names
    }

    // Natijalar modalida bitta PatientDiagnosis bitta varaq bo'ladi.
    // Ichidagi analizlar jadvalda alohida sarlavha qatorlari bilan ajraladi.
    const splitDiagnosisIntoCategories = (diagnosis, patientOverride = null) => {
        const saved = diagnosis.results
        const cols = (saved?.columns?.length > 0) ? saved.columns : [...defaultColumns]
        const sourceItems = getResultSourceItems(diagnosis)

        // categoryId bo'lmagan elementlar uchun findDiagnosisByRef orqali kategoriyani topamiz
        sourceItems.forEach(item => {
            if (!item.categoryId) {
                const match = findDiagnosisByRef(item.id, item.displayName || item.name, null)
                const resolvedCatId = getDiagnosisCategoryId(match)
                if (resolvedCatId) item.categoryId = resolvedCatId
            }
        })

        const tableNames = getResultTableNames(diagnosis, sourceItems)
        const tableTitle = tableNames[0] || 'Natija'
        const sidebarName = tableNames.join(', ') || diagnosis.diagnosisName || tableTitle

        return [{
            diagnosisId: diagnosis._id,
            diagnosisName: sidebarName,
            createdAt: diagnosis.createdAt,
            title: tableTitle,
            columns: cols,
            rows: buildAutoRows(diagnosis.diagnosisName, patientOverride, saved?.rows || [], cols, diagnosis),
    ...
    const openResultsEntryModal = async () => {
        ...
        const allCategoryTables = patientDiagnoses.flatMap(d => splitDiagnosisIntoCategories(d))
        setCurrentCategoryIndex(0)
        setCategoryResultsAndRef(allCategoryTables)
        setShowResultsEntryModal(true)
    }

    // Bitta kategoriya jadvalini ochish (kategoriyalarga ajratib)
    const openSingleCategoryModal = (diagnosis) => {
        setSelectedDiagnosisForResults(diagnosis)
        setCurrentCategoryIndex(0)
        const catTables = splitDiagnosisIntoCategories(diagnosis)
        setCategoryResultsAndRef(catTables)
        setShowResultsEntryModal(true)
    }
    ...
    const saveResults = async (options = {}) => {
        const confirmResults = options?.confirm === true
        ...
        try {
            ...
            // Bir xil diagnosisId ga ega kategoriyalarni birlashtirib saqlaymiz
            const grouped = {}
            current.forEach(cat => {
                if (!grouped[cat.diagnosisId]) grouped[cat.diagnosisId] = []
                grouped[cat.diagnosisId].push(cat)
            })

            for (const [diagnosisId, categories] of Object.entries(grouped)) {
                // Barcha kategoriyalar satrlarini birlashtirish
                const mergedRows = []
                const conclusions = []
                categories.forEach(cat => {
                    const validRows = cat.rows
                        .filter(r => Object.values(r.values || {}).some(v => v !== undefined && v !== null && v.toString().trim() !== ''))
                        .map(r => ({ values: r.values }))
                    mergedRows.push(...validRows)
                    if (cat.conclusion && cat.conclusion.trim()) {
                        conclusions.push(cat.conclusion.trim())
                    }
                })

                const response = await fetch(
                    `/api/patient-diagnoses/${diagnosisId}/results`,
                    {
                        method: 'PUT',
                        headers: {
                        ...
                        body: JSON.stringify({
                            ...
                            columns: categories[0].columns,
                            rows: mergedRows,
                            conclusion: conclusions.join('\n'),
                            isConfirmed: confirmResults
                        })
                    }
                )
            ...
            if (allSaved) {
                showToast(confirmResults ? 'Natijalar tasdiqlandi!' : 'Natijalar muvaffaqiyatli saqlandi!')
                fetchPatients()
                return true
            } else {
    ...
    const handlePrintAndSaveResults = (e) => {
        if (e?.preventDefault) e.preventDefault()
        const allCats = categoryResultsRef.current
        if (!allCats || allCats.length === 0) return
        const catsToPrint = [allCats[currentCategoryIndex] || allCats[0]].filter(Boolean)
        ...
        // Faqat joriy analiz qatorlarini yig'ib, diagnosesList orqali kategoriyaga bog'lash
        ...
        catsToPrint.forEach(cat => {
            ...
            const idName = cols[0]?.id || 'col_1'
            const sourceDiagnosis = patientDiagnoses.find(d => d._id === cat.diagnosisId) || cat
            const rows = getPrintableResultRows(sourceDiagnosis, cols, cat.rows || [])
            ...
            sortedRows.forEach(row => {
                const catName = getResultRowCategoryName(row, idName, cat.title || 'Boshqa')
                if (!groupMap[catName]) {
                    groupMap[catName] = { rows: [], cols, createdAt: cat.createdAt, conclusion: cat.conclusion, sourceDiagnosis }
        ...
        const firstCreatedAt = groupMap[groupOrder[0]]?.createdAt || now
        const firstDailyNum = groupOrder.map(catName => groupMap[catName]?.sourceDiagnosis?.dailyNumber).find(Boolean) || selectedPatient?.dailyNumber
        const pages = groupOrder.map((catName) => {
            const { rows, cols, sourceDiagnosis } = groupMap[catName]
            const dailyNum = sourceDiagnosis?.dailyNumber || selectedPatient?.dailyNumber || ''
            return `
        ...
        printWindow.document.write(`<!DOCTYPE html><html><head>
            ...
            <div class="print-page">
                ...
                ${buildPatientBlock(firstCreatedAt, now, firstDailyNum)}
                ${pages}
            </div>
        </body></html>`)
    ...
    const getPrintStyle = () => `
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .result-section:not(:last-of-type) .print-footer {
            display: none;
        }
        ...
        .results-table tr.out-of-range td.result-val {
    ...
    const buildPatientBlock = (regDate, readyDate, customDailyNum) => {
        ...
        const dailyNum = customDailyNum || p?.dailyNumber || 1
        ...
        return `
            <div class="print-patient">
                <div class="print-patient-col">
                    ...
                    <span><b>Тел:</b> ${p?.phone || '-'}</span>
                    <span><b>Врач:</b> ${p?.referredBy || '-'}</span>
                </div>
                ...
                <div class="print-patient-col">
                    ...
                    <span><b>Пол:</b> ${p?.gender === 'male' ? 'Erkak' : 'Ayol'}</span>
                </div>
            </div>
        `
    }
    ...
    const getPrintableResultRows = (source, cols, rowsOverride = null) => {
        const idName = cols[0]?.id || 'col_1'
        const visibleColIds = cols.map(c => c.id)
        const rows = (rowsOverride || source?.results?.rows || []).filter(row =>
        ...
        const sourceItems = getResultSourceItems(source)
    ...
    const buildTableHTML = (columns, rows, groupByCategory = false, sourceDiagnosis = null) => {
        ...
        const normalizedRows = normalizeSavedResultRows(rows, columns, sourceDiagnosis)
        ...
        if (groupByCategory) {
            ...
            groups.forEach(group => {
                group.rows.forEach((row, ri) => {
                    ...
                    const isOut = row.values?.isAbnormal === true || isOutOfRange(resultVal, normaVal)
                    const catCell = ri === 0
                        ...
                        : ''
                    bodyRows += `<tr class="${isOut ? 'out-of-range' : ''}">${catCell}${columns.map((col, ci) => `<td class="${ci === 1 ? 'result-val' : ''}">${formatPrintCell(row.values?.[col.id])}</td>`).join('')}</tr>`
                })
            })
        ...
        const bodyRows = normalizedRows.map(row => {
            ...
            const normaVal  = normaColId ? (row.values?.[normaColId] || '') : ''
            const isOut = row.values?.isAbnormal === true || isOutOfRange(resultVal, normaVal)
            return `<tr class="${isOut ? 'out-of-range' : ''}">${columns.map((col, ci) => `<td class="${ci === 1 ? 'result-val' : ''}">${formatPrintCell(row.values?.[col.id])}</td>`).join('')}</tr>`
        }).join('')
    ...
    const handlePrintSavedResults = (diagnosis) => {
        ...
        const groupMap = {}
        const groupOrder = []
        const filteredRows = getPrintableResultRows(diagnosis, cols)
        ...
        const pages = groupOrder.map((catName, pageIdx) => {
            ...
            return `
                <div class="result-section">
                    ...
                    ${buildTableHTML(pageCols, rows, false, diagnosis)}
        ...
        printWindow.document.write(`<!DOCTYPE html><html><head>
            ...
            <div class="print-page">
                ${buildPrintHeader(logoUrl)}
                ${buildPatientBlock(diagnosis.createdAt, now, diagnosis.dailyNumber)}
                ${pages}
            </div>
        </body></html>`)
        printWindow.document.close()
    ...
    const addResultRow = (categoryIndex) => {
        setCategoryResultsAndRef(prev => prev.map((cat, idx) => {
            if (idx === categoryIndex) {
                const newValues = {
                    _categoryName: cat.title || cat.diagnosisName || 'Laboratoriya tahlili'
                }
    ...
    const updateResultRow = (categoryIndex, rowId, colId, value) => {
        setCategoryResultsAndRef(prev => prev.map((cat, idx) => {
            if (idx === categoryIndex) {
                ...
                const isNameCol = colId === idName
                return {
                    ...cat,
                    rows: cat.rows.map(row => {
                        if (row.id === rowId) {
                            let extra = {}
                            if (isNameCol && value.trim()) {
                                const match = findDiagnosisByRef(null, value.trim())
                                if (match) {
                                    extra = {
                                        _diagnosisId: toId(match._id),
                                        _categoryId: getDiagnosisCategoryId(match),
                                        _categoryName: getDiagnosisCategoryName(match)
                                    }
                                }
                            }
                            return { ...row, values: { ...row.values, [colId]: value, ...extra } }
                        }
                        return row
                    })
                }
    ...
    const handlePrintAllCategoryResults = () => {
        ...
        const categoriesHTML = allCats.map((category) => {
        ...
        printWindow.document.write(`<!DOCTYPE html><html><head>
            ...
            ${buildPrintHeader(logoUrl)}
            ${buildPatientBlock(now, now, selectedPatient?.dailyNumber)}
            ${firstCat ? `
            <div class="print-title-row">
                <div class="print-title">${firstCat.title || 'LABORATORIYA TAHLILI'}</div>
            </div>
            ${buildTableHTML(firstCat.columns || [], (firstCat.rows || []).filter(r => Object.values(r.values || {}).some(v => v && v.toString().trim())), false, firstCat)}
            ...
            ${allCats.slice(1).map((category, i) => {
                ...
                return `
                    ...
                    ${buildTableHTML(columns, rows, false, category)}
    ...
    const handleViewAnalyzes = async (patient) => {
        ...
        try {
            ...
            const tables = diagnoses.flatMap(d => splitDiagnosisIntoCategories(d, patient))
    ...
    const openDiagnosisModal = () => {
        ...
        const base = getBase()
        navigate(`${base}/patients/diagnosis/${selectedPatient._id}?new=1`)
    }
    ...
    const getDiagnosisDisplayItems = (diagnosis) => getResultSourceItems(diagnosis)
    ...
    const getReceiptItems = (diagnosis) => {
        ...
        const isReceiptInternalItem = (item) => {
            const categoryCode = (getCategoryById(item.categoryId)?.code || '').toString().trim().toLowerCase()
            const categoryName = (item.categoryName || getCategoryById(item.categoryId)?.name || '').toString().trim().toLowerCase()
            return categoryCode === 'mom' || categoryName === 'микроскопия осадка мочи'
        }

        if (prices.length === 0) {
            const fallbackPrice = Number(diagnosis?.diagnosis?.price || 0)
        ...
        const items = prices
            .map(p => {
                ...
                return {
                    ...
                    categoryId,
                    categoryName: p.categoryName || getDiagnosisCategoryName(match) || categoryMeta.categoryName || 'Laboratoriya',
    ...
    const getReceiptItemsTotal = (items = []) => {
        return items.reduce((sum, item) => sum + Number(item?.price || 0), 0)
    }

    const getDiagnosisDisplayTotal = (diagnosis) => getReceiptItemsTotal(getReceiptItems(diagnosis))

    // Category helper funksiyalar (doctor uslubida)
    const getDiagnosesByCategory = (categoryId) =>
        diagnosesList
            .filter(d => d.category === categoryId || d.category?._id === categoryId)
        ...
        try {
            ...
            const response = await fetch('/api/patient-diagnoses', {
                ...
                body: JSON.stringify({
                    ...
                    diagnosisPrices: diagnosisFormData.diagnoses.map(d => {
                    ...
                    totalAmount: Math.max(0, diagnosisFormData.diagnoses.reduce((s, d) => s + (d.price || 0), 0) - (paymentData.discount || 0)),
            ...
            if (response.ok) {
                ...
                const totalAmount = data.totalAmount ?? Math.max(0, diagnosisFormData.diagnoses.reduce((s, d) => s + (d.price || 0), 0) - (paymentData.discount || 0))
    ...
    const handlePaymentSubmit = async () => {
        ...
        try {
            const token = localStorage.getItem('token')
            const totalAmount = Math.max(0, paymentData.consultationFee + paymentData.medicinesCost - paymentData.discount)
    ...
    const handlePrintPaymentReceipt = (diagnosis) => {
        ...
        const diagnosisItems = getReceiptItems(diagnosis)
        const diagnosisTotal = getReceiptItemsTotal(diagnosisItems)
        const savedDiscount = Number(diagnosis.discount || 0)
        ...
        const totalAmount = Math.max(0, diagnosisTotal + medicinesCost - savedDiscount)
...
<body>
    ...
    ${savedDiscount > 0 ? `<div class="discount-row">Chegirma ${diagnosis.discountPercent > 0 ? `(${diagnosis.discountPercent}%)` : ''}: \u2212 ${savedDiscount.toLocaleString()} so'm</div>` : ''}
    ...
    const handlePrintCombined = (diagnosisData, payment, totalAmount) => {
        ...
        const receiptDiagnosisItems = getReceiptItems(diagnosisData)
        const diagnosisItems = receiptDiagnosisItems.length > 0
            ? receiptDiagnosisItems
            : diagnosisFormData.diagnoses.map(d => ({ name: d.diagnosisName, price: d.price || 0 }))

        // Kategoriyalar (analizlar) narxi
        const categoriesTotal = diagnosisItems.reduce((sum, d) => sum + (d.price || 0), 0)
        ...
        const grandTotal = Math.max(0, categoriesTotal + medicinesTotal - Number(payment?.discount || 0))
...
<body>
    ...
    <div class="section">
        ...
        ${diagnosisItems.map(d => `<div class="item-row"><span class="item-name">${d.name || d.diagnosisName}</span><span class="item-price">${(d.price||0).toLocaleString()} so'm</span></div>`).join('')}
    ...
    const handlePrintReceipt = () => {
        ...
        const now = new Date()
        const receiptNumber = `CHK-${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}${now.getDate().toString().padStart(2, '0')}-${now.getHours().toString().padStart(2, '0')}${now.getMinutes().toString().padStart(2, '0')}${now.getSeconds().toString().padStart(2, '0')}`
        const receiptDiagnosisLabel = getReceiptItems(lastSavedDiagnosis).map(item => item.name).join(', ') || lastSavedDiagnosis?.diagnosisName || '-'
            ...
            <body>
                <div class="receipt">
                    ...
                    <div class="receipt-info">
                        <div class="receipt-row"><span>Sana:</span><span>${now.toLocaleDateString('uz-UZ')} ${now.toLocaleTimeString('uz-UZ')}</span></div>
                        <div class="receipt-row"><span>Bemor:</span><span>${selectedPatient?.fullName}</span></div>
                        <div class="receipt-row"><span>Analiz:</span><span>${receiptDiagnosisLabel}</span></div>
                    </div>
                    ...
                    <div class="receipt-totals">
                        ...
                        ${paymentData.discount > 0 ? `<div class="receipt-total-row"><span>Chegirma ${paymentData.discountPercent > 0 ? `(${paymentData.discountPercent}%)` : ''}:</span><span>-${paymentData.discount.toLocaleString()} so'm</span></div>` : ''}
    ...
    const isActiveToday = (patient) => {
        ...
        // Check latest diagnosis date (from PatientDiagnosis records)
        if (isToday(patient.latestDiagnosisDate)) {
            return true
        }
        return false
    }
                                    ...
                                    <div className="pv-section-actions">
                                        ...
                                        {getBase() !== '/doctor' && (
                                            <button className="btn btn-primary btn-sm" onClick={() => {
                                                navigate(`${getBase()}/patients/diagnosis/${selectedPatient._id}?new=1`)
                                            }}>
                                                <PlusCircle size={16} />
                                                analiz qo'shish
                                            </button>
                                ...
                                {diagnosesLoading ? (
                                    ...
                                    <div className="pv-diagnoses-list">
                                        {patientDiagnoses.map((diagnosis, idx) => {
                                            const items = getDiagnosisDisplayItems(diagnosis)
                                            const displayTotal = getDiagnosisDisplayTotal(diagnosis)
                                            return (
                                                    ...
                                                    <div className="pv-dc-top">
                                                        ...
                                                        <div className="pv-dc-tags">
                                                            {items.map((item, itemIdx) => (
                                                                <span className="pv-dc-tag" key={`${diagnosis._id}-${item.code || item.displayName}-${itemIdx}`}>
                                                                    {item.displayName}
                                                                </span>
                                                            ))}
                                                            <span className="pv-dc-tag" style={{ background: '#eff6ff', borderColor: '#bfdbfe', color: '#1d4ed8', fontWeight: '600' }}>
                                    ...
                                    <div className="df-modal-body">
                                        ...
                                        <div className="df-modal-sidebar">
                                            <div className="df-cat-list">
                                                ...
                                                {categoriesList.filter(cat => !isInternalResultCategory(cat)).map(cat => {
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        const subtotal = diagnosisFormData.diagnoses.reduce((sum, d) => sum + (d.price || 0), 0) +
                                                            diagnosisFormData.diagnoses.flatMap(d => d.medicines).reduce((sum, m) => sum + (getMedicinePrice(m.medicine) * (m.quantity || 1)), 0)
                                                        setPaymentData({
                                                            ...paymentData,
                                                            discountPercent: percent,
                                                            discount: calculateDiscountAmount(subtotal, percent)
                                                        })
                                                    }}
                                                    style={{ width: '80px' }}
                                                    className="form-input"
                            ...
                            <div className="payment-summary">
                                ...
                                <div className="summary-row discount">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const percent = clampDiscountPercent(e.target.value)
                                            const subtotal = paymentData.consultationFee + paymentData.medicinesCost
                                            setPaymentData({
                                                ...paymentData,
                                                discountPercent: percent,
                                                discount: calculateDiscountAmount(subtotal, percent)
                                    ...
                                    {paymentData.discount > 0 && (
                                        <span style={{ color: '#e53e3e', fontSize: '12px' }}>
                                            (−{paymentData.discount.toLocaleString()})
                                        </span>
                                    )}
                                </div>
                                <div className="summary-row total">
                                    <span>JAMI TO'LOV:</span>
                                    <span>{Math.max(0, paymentData.consultationFee + paymentData.medicinesCost - paymentData.discount).toLocaleString()} so'm</span>
                                </div>
                            </div>
                ...
                <div className="results-fullpage">
                    ...
                    <div className="rfp-header">
                        ...
                        <div className="rfp-header-actions">
                            ...
                            <button className="rfp-btn rfp-btn-confirm" onClick={() => saveResults({ confirm: true })}>
                                <Check size={16} /> Tasdiqlash
                            </button>
                            <button type="button" className="rfp-btn rfp-btn-print" onClick={handlePrintAndSaveResults}>
                                        ...
                                        <div className="rfp-table-wrap">
                                            <table className="rfp-table">
                                                ...
                                                <tbody>
                                                    {(() => {
                                                        const idName = (cat.columns || [])[0]?.id || 'col_1'
                                                        const idResult = (cat.columns || [])[1]?.id || 'col_2'
                                                        const isResult = (colId) => colId === idResult

                                                        // Har bir qatorni kategoriyasiga bog'lash
                                                        const rowsWithCat = (cat.rows || []).map(row => {
                                                            const catName = getResultRowCategoryName(row, idName, cat.title || 'Laboratoriya tahlili')
                                                            return { row, catName }
                                                        })

                                                        // Kategoriya bo'yicha guruhlab sortlash
                                                        const sortedRowsWithCat = [...rowsWithCat].sort((a, b) => {
                                                            const idxA = categoriesList.findIndex(c => c.name === a.catName)
                                                            const idxB = categoriesList.findIndex(c => c.name === b.catName)
                                                            if (idxA !== -1 && idxB !== -1) return idxA - idxB
                                                            if (idxA !== -1) return -1
                                                            if (idxB !== -1) return 1
                                                            if (a.catName && b.catName) return a.catName.localeCompare(b.catName)
                                                            if (a.catName) return -1
                                                            if (b.catName) return 1
                                                            return 0
                                                        })

                                                        let dataRowIdx = 0
                                                        let lastCat = '__NONE__'
                                                        return sortedRowsWithCat.map(({ row, catName }, i) => {
                                                                            ...
                                                                            return (
                                                                                <td key={col.id} className={isResultCol ? 'rfp-td-result' : 'rfp-td-editable'}>
                                                                                    <textarea
                                                                                        ...
                                                                                        data-row={currentDataIdx}
                                                                                        data-col={colIdx}
                                                                                        onKeyDown={e => {
                                                                                            if (e.key === 'Enter' && !e.shiftKey && isLastRow && isResultCol) {
                                                                                                e.preventDefault()
                                                                                                ...
                                                                                                e.preventDefault()
                                                                                                const nextInput = document.querySelector(`.rfp-cell-input[data-row="${currentDataIdx + 1}"][data-col="${colIdx}"]`)
                                                                                                if (nextInput) nextInput.focus()
                                                                                            } else if (e.key === 'ArrowUp') {
                                                                                                e.preventDefault()
                                                                                                const prevInput = document.querySelector(`.rfp-cell-input[data-row="${currentDataIdx - 1}"][data-col="${colIdx}"]`)
